...
export function DoorScene({ color = "#383e42", variant = 0, title }: { color?: string; variant?: number; title?: string }) {
  const styles: SurfaceStyle[] = ["MODERN", "CLASSIC", "MINIMAL", "LOFT", "NEOCLASSIC", "MODERN"];
  return <div role="img" aria-label={title} aria-hidden={title ? undefined : true} className="relative h-full min-h-0 w-full overflow-hidden" style={{ background: variant % 2 ? "#ddd5c7" : "#e8e4dc" }}>
