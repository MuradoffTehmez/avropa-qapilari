...
export function DataTable<T>({
  ...
  return (
    <Card>
      ...
      <Modal open={editIndex!==null} onClose={() => setEditIndex(null)} title="Demo qeydini redaktə et"><form className="space-y-3" onSubmit={(e) => { e.preventDefault(); state.save(title, data.map((r,i) => i===editIndex ? draft : r)); setEditIndex(null); toast("Demo dəyişiklikləri saxlanıldı"); }}>{Object.entries(draft).filter(([key,value]) => key!=="id" && (typeof value==="string" || typeof value==="number" || typeof value==="boolean")).map(([key,value]) => <label key={key} className="block text-sm">{editLabels[key] ?? key}{typeof value==="boolean" ? <input className="ml-3" type="checkbox" checked={value} onChange={(e) => setDraft({...draft,[key]:e.target.checked})}/> : <input className="mt-1 w-full border p-2" type={typeof value==="number" ? "number" : "text"} step="any" value={String(value)} onChange={(e) => setDraft({...draft,[key]:typeof value==="number" ? Number(e.target.value) : e.target.value})}/>}</label>)}<Button type="submit">Yadda saxla</Button><p className="text-xs text-stone">Dəyişiklik yalnız bu demo cədvəlində və brauzerdə saxlanılır.</p></form></Modal>
    </Card>
  );
}
